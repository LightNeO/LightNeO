# 👋 Hi, I'm Anton Yeromin

🧑‍💻 Senior QA Engineer with 5+ years in web, desktop, mobile and game testing  
🌍 Platforms: Windows, MacOS, Linux, Android, iOS, Steam Deck, VR

---

## 🛠 Skills
- Manual Testing: Functional, Regression, UI/UX, Exploratory, Integration
- Automation: Python (Selenium, PlayWright, Pywinauto), C# (basics)
- Tools: Jira, TestRail, Postman, Charles Proxy, Git, Azure, PerfDog, MSI Afterburner and many others
- Build Distribution: HockeyApp, AppCenter, TestFlight, Crashlytics
- Soft Skills: Communication, Ownership Mindset, Problem Solving
- Methodologies: Scrum, Kanban

---

## 💼 Experience
- **QA Engineer** (May 2020 – Present) grew from an Intern to a Senior.
- Led QA efforts for projects across different platforms. Created documentation, mentored juniors, participated in agile processes, ensured build quality.

---

## 🎓 Education
- 🎓 Sumy National Agrarian University — Bachelor (2018)
- 🧠 Prometheus Academy — QA Automation with Python
- 🔧 Currently participating in the real desktop product automation using PyWinAuto
- 🌍 Designed a [website](https://retro-gaming-store.onrender.com/) for practising QA Auto skills

---

## 🗣️ Languages
- Ukrainian — Native  
- English — Upper Intermediate

---

## 📫 Contact
📧 antonereminsumy@gmail.com  
📞 +380 95 183 3435  
